...
### 1. `TotalRulesEvaluated` is always 0
**File:** `Implementation.cs` — `Evaluate()` method
**Issue:** `totalRules` is initialized to 0 and never incremented. Every `IEvaluationResult` reports zero rules evaluated regardless of actual work done.
**Fix:** Increment `totalRules` by the rule count for each fact type evaluated.
...
**Issue:** `FactQueryDetector.VisitConstant` detects `FactQueryable<T>` in constant nodes and `VisitMember` evaluates member access to find captured `FactQueryable<T>` instances. `FactQueryRewriter` does NOT override `VisitConstant` or `VisitMember`, so it can't rewrite those same nodes. If an expression tree hits this pattern (closure-captured `FactQueryable<T>` that isn't unwrapped by `Rule<T>`'s own detection), detection says "needs rewriting" but rewriting silently skips the node → runtime produces wrong results.
**Risk:** Currently mitigated because `Rule<T>` handles its own closure detection separately. But Path 3 (generic rewriter dispatch) uses `ContainsFactQuery()` for detection and the session's `FactQueryRewriter` for rewriting — this is the mismatch path.
**Fix:** `FactQueryRewriter` should handle the same node types as `FactQueryDetector`.
...
| **P0** | #4 Detector/Rewriter asymmetry | Silent wrong results on Path 3 |
...
| **P1** | #1 TotalRulesEvaluated always 0 | Broken metrics |
